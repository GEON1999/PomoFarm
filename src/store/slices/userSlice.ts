...
const userSlice = createSlice({
  ...
  reducers: {
    ...
    updateProfile: (state, action: PayloadAction<{ username?: string; email?: string }>) => {
    ...
    addToInventory: (state, action: PayloadAction<{item: Omit<InventoryItem, 'quantity'>, quantity?: number}>) => {
    ...
    removeFromInventory: (state, action: PayloadAction<{itemId: string, quantity?: number}>) => {
...
export const removeFromInventoryWithCheck = createAsyncThunk(
  ...
  async (payload: { itemId: string; quantity?: number }, { getState, dispatch }) => {
    const state = getState() as RootState;
    const { itemId, quantity = 1 } = payload;
    const item = state.user.inventory.find((i: InventoryItem) => i.id === itemId);
